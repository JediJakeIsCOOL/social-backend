...
router.get('/viewprofilefeed/:username/:feedtype/:userid', (req, res) => {
	...
	if(req.params.feedtype === '0'){
		...
		`SELECT gamelink_feedposts.*, gamelink_users.gamelink_username, gamelink_users.gamelink_userpic, games.game_title, platforms.name AS platform_name, game_ratings.game_review, 
		game_ratings.rating, IFNULL(t2.commentCount, 0) AS commentCount
		...
		where gamelink_users.gamelink_username = '${req.params.username}' ORDER BY gamelink_feedposts.gamelink_feedpostid DESC`
	} else if (req.params.feedtype === '1') {
		var retreiveProfileInfo = 
		`SELECT gamelink_feedposts.*, gamelink_users.gamelink_username, gamelink_users.gamelink_userpic, games.game_title, platforms.name AS platform_name, 
		game_ratings.game_review, game_ratings.rating, IFNULL(t2.commentCount, 0) AS commentCount
		FROM gamelink_dev_db.gamelink_feedposts
		JOIN gamelink_dev_db.gamelink_users ON gamelink_users.gamelink_userid = gamelink_feedposts.gamelink_userid
		LEFT JOIN thegamesdb_db.games ON gamelink_feedposts.gamelink_gameid = games.id
		left join gamelink_dev_db.gamelink_usersgames ON gamelink_usersgames.gamelink_gameid = gamelink_feedposts.gamelink_gameid
		...
		LEFT JOIN thegamesdb_db.platforms ON games.platform = platforms.id
		LEFT JOIN thegamesdb_db.game_ratings ON gamelink_feedposts.gamelink_gameid = game_ratings.game_id AND gamelink_feedposts.gamelink_userid = game_ratings.game_userid
		where gamelink_users.gamelink_username = '${req.params.username}' ORDER BY gamelink_feedposts.gamelink_feedpostpoints DESC`
		...
		LEFT JOIN thegamesdb_db.platforms ON games.platform = platforms.id
		LEFT JOIN thegamesdb_db.game_ratings ON gamelink_feedposts.gamelink_gameid = game_ratings.game_id AND gamelink_feedposts.gamelink_userid = game_ratings.game_userid
		where gamelink_users.gamelink_username = '${req.params.username}' and gamelink_feedposts.gamelink_isreview = 1 ORDER BY gamelink_feedposts.gamelink_feedpostid DESC `
		...
		JOIN gamelink_dev_db.gamelink_users ON gamelink_users.gamelink_userid = gamelink_feedpostcomments.gamelink_userid_comment
		LEFT JOIN thegamesdb_db.games ON gamelink_feedposts.gamelink_gameid = games.id
		left join gamelink_dev_db.gamelink_usersgames ON gamelink_usersgames.gamelink_gameid = gamelink_feedposts.gamelink_gameid
		LEFT JOIN thegamesdb_db.platforms ON games.platform = platforms.id
		WHERE gamelink_users.gamelink_username = '${req.params.username}'
